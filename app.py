...
def index():
    #讀取檔案
    namestock=[]
    with open("stock.txt",mode="r",encoding="utf-8") as file:
        for line in file:
            namestock.append(line.strip('\n') )#year單格資料

    if request.method == "POST":
        stock = request.form["nm1"] #nm是html提交表格nmae
        return redirect(url_for("get_table",stock_id=stock)) #url_for,後面要是函數
    return render_template("index.html",namestock=namestock)#新增測試,namestock=namestock
...
def get_table(stock_id):
  ...
  #----------------  
    ...
    #-------------------------------------------
    # 最新股價
    data555=soup.select("table.b1.p4_2.r10")  #使用bs4以網頁程式碼找出位置
    data88=re.findall('(\S+)',data555[0].text)#\S: 非空格字元
    nowprice=data88[11]
    #-------------------------------------------
    ...
    return render_template("test.html",stockname=stockname,nowprice=nowprice,nmp_t0=nmp_t0,nmp_n0=nmp0_n,len_0=len_0)
...
def get_tab01(url,headers):#goodinfo經營績效
    ...
    df71 = df6[df6['最高價'] != '-']#把不要消除
    df8 = df71[df71['現金股利'] != '現金股利']#把不要消除  
    ...
    for i in range(0,f): #range(x, y) #for 下一列要縮進喔 range(0,6)整數數列：0,1,2,3,4,5
        if df9['現金股利'].loc[i]=="-":#配息還沒出來
            Dividendcash.append("0")#year單格資料
            Dividendstcok.append("0")#year單格資料
            cashperson.append("0")#year單格資料
    ...
    stockcost=[]
    stockcost.append(round(float(nowprice)*(1+(Dividendstcok[0]/10))+Dividendcash[0],1))#highprice單格資料
    for i in range(0,len(Dividendcash)-1):#合計股息
        stockcost.append(round(float(stockcost[i])*(1+(Dividendstcok[i+1]/10))+Dividendcash[i+1],1))#highprice單格資料
    for i in range(0,len(Dividendcash)):#合計股息
        ...
        df9['還原權值'][i]=round(stockcost[i],1)#(round(x,1))留小數1位PE1 #去年本益比
